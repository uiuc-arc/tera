...
def relative_error(x, y, atol=0):
    error = abs(x - y)
    # ind=np.where(np.any(y>0))
    #max_abs_error = max(error[ind])
    if y == 0:
        return np.inf
...
def is_min_bound(assert_spec):
    ...
    if assert_type in [AssertType.ASSERTLESS, AssertType.ASSERTLESSEQUAL, AssertType.ASSERT_ARRAY_LESS] and assert_spec.reverse:
        return True
    elif assert_type in [AssertType.ASSERTTRUE, AssertType.ASSERTFALSE, AssertType.ASSERT] \
        ...
        return True
    elif assert_type in [AssertType.ASSERTTRUE, AssertType.ASSERTFALSE, AssertType.ASSERT] and '>' in assert_str and not assert_spec.reverse:
        return True
...
def bootstrap_CI(samples, npstat=np.min, alpha =0.95, resamples = 1000, resample_ratio=0.5):
    ...
    #print("CI-Mean: {0} +- {1}".format(np.mean(ordered), (upper-lower)/2))
