...
class CompareDistribution:
        ...
        for arg in args:
            if 'decimal' in arg or 'significant' in arg:
                ...
                if self.spec.assert_type == AssertType.ASSERT_ALMOST_EQUAL:
                    # may need to differentiate later
    ...
    def compute_fit_score(self, samples, models=None):
        ...
        bin_width = x[1] - x[0]
        N = len(samples)
        # go from bin edges to bin middles
    ...
    def compute_best_extreme_dist(self, samples, max_bound=True, min_bound=False):
        try:
            ...
            self.logger.logo(
                "Cannot compute variance, continuing with distribution computation...")
        #if max_bound:
        #    models = [dist.gumbel_r, dist.frechet_r, dist.weibull_max]
        #else:
        #    models = [dist.gumbel_l, dist.frechet_l, dist.weibull_min]
    ...
    def parse(self):
        with open(self.sample_file, 'r') as f:
            lines = f.readlines()
            #self.spec.assert_type = AssertType.get_assert_type(
            #    lines[6].split(' ')[1].split('(')[0].split('.')[-1])
    ...
    def evaluate(self):
        ...
        # elif self.spec.assert_type in [AssertType.ASSERT_ARRAY_ALMOST_EQUAL]:
        #     p = self.check_assert_relative_tolerance(actual, expected, default_threshold.get(self.spec.assert_type))
    ...
    def check_assert_equal(self, actual: list, expected: list):
        ...
        #         p = self.check_assert_equal([x[ind] for x in actual], expected if Util.getdims(
        #             expected) == 1 else [t[ind] for t in expected])
        #         probs.append(p)
    ...
    def check_assert_tolerance(self, actual: list, expected: list, tol_thresh):
        # if Util.getdims(actual) > 2:
        #     print(actual)
        #     print("Higher dimension! Cannot handle")
        #     return
        if isinstance(tol_thresh, dict):
            if 'rtol' in tol_thresh:
                tol_thresh = tol_thresh['rtol']
    ...
    # get distribution limit, to decide the new threshold
    ...
    def get_new_bound_gev(self, dist, max_percentile):
        ...
        if self.spec.assert_type in [AssertType.ASSERTLESS, AssertType.ASSERTLESSEQUAL]:
            if self.spec.reverse:
                ...
                #b, e = self.get_assert_greater_bound(actual, expected, threshold)
            else:
    ...
    def get_assert_greater_bound(self, actual, expected, threshold):
        ...
        #     if var <= 0.0:
        #         var = 1e-20
        #     bound = norm.cdf(expected[0], loc=mean, scale=var)
        # else:
        #     ecdf = ECDF(actual)
        #     bound = ecdf([expected[0]])[0]
    ...
    def check_sample_size(self, samples):
        ...
        # print(return_levels)
        # sw_stats = [AD_pval(st.anderson(t, 'norm')[0], len(t)) for t in return_levels]  # considering only p value
    ...
    def llrtest(self, samples, alpha=0.05):
        ...
        #print("p val", 1 - st.chi2(1).cdf(llr))
        #print("ppf-gen: {0}, ppf-exp: {1}".format(d1.ppf(0.9999), d2.ppf(0.9999)))
        if llr <= chi:
    ...
    def run_mbpta_cv(self, samples, min_tail_values, max_percentile, printf=None):
        ...
        for th in thresholds:
            ...
            # cv_th = np.std(exc) / np.mean(exc)
            # gpd_param = st.genpareto.fit(exc)
            ...
            if pval > 0.05:
                #printf("Threshold accepted! GPD Test passed")
                th_dict[th] = (pval, shape, scale)

                # if shape <= 0:
                #     is_exp_tail = True
                #     exp_params = st.genpareto.fit(exc)
                ...
                if tail_type == TailType.LIGHT or tail_type == TailType.EXP:
                    ...
                    #printf("LLR test passed")

        #printf("Exp Tail : {0}".format(is_exp_tail))
    ...
    # get best ppf estimate when distribution does not converge (under approximates)
    # adjust_ppf: choose lower percentile when cv < 0.7
    def get_best_fit_exp_tail_estimate(self, samples, min_tail_values, max_percentile, printf=None, adjust_ppf=False):
        ...
        #thresholds = np.random.choice(thresholds, 20)
        printf("Thresholds : {0}".format(thresholds))
        # get cv threshold closest to 1
        for th in thresholds:
            s = samples[samples > th]
            if len(s) < 50:
                continue
        ...
        if np.isfinite(best_th):
            printf("Computing exp dist")
            # if adjust_ppf and best_cv < 0.7:
            #     printf("Best cv : {0}, Using lower ppf".format(best_cv))
            #     estimate = self.fit_expon_to_tail(best_th, 0.99, printf, samples)
            # else:
            estimate = self.fit_expon_to_tail(best_th, max_percentile, printf, samples)
            printf(estimate)
            return estimate
    ...
    def fit_expon_to_tail(self, best_th, max_percentile, printf, samples):
        ...
        # estimate difference between pareto and exp
...
if __name__ == '__main__':
    ...
    logdir = sys.argv[1]
    sampleFiles = glob.glob(os.path.join(
        logdir, '**/samples.txt'), recursive=True)
    for sampleFile in sampleFiles:
        print(sampleFile)
        cd = CompareDistribution(sampleFile)
        cd.evaluate()
    # print(sampleFiles)
